...
describes what users get. Pick a format with `-o`, or let ena choose:
a table when writing to a terminal, JSONL when piped.

## Formats

```bash
ena <command> -o table   # aligned columns for reading
ena <command> -o jsonl   # one JSON object per line, for piping
ena <command> -o json    # a single JSON array
ena <command> -o csv     # spreadsheet friendly
ena <command> -o tsv     # tab-separated
ena <command> -o url     # just the URL column
ena <command> -o raw     # the underlying bytes, unformatted
```
...
```bash
ena <command> --fields id,title,url
```
...
```bash
ena <command> --template '{{.URL}} {{.Title}}'
```
...
```bash
ena <command>            # a table, because this is a terminal
ena <command> | wc -l    # JSONL, because this is a pipe
```
